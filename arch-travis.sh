...
ARCH_TRAVIS_MIRRORS=${ARCH_TRAVIS_MIRRORS:-"https://mirrors.ocf.berkeley.edu/archlinux,https://mirror.csclub.uwaterloo.ca/archlinux,https://mirror.lty.me/archlinux,http://mirrors.kernel.org/archlinux/"}
...
takedown_chroot() {
  ...
  for d in $HOME/*/; do
    if [ "$d" != "$ARCH_TRAVIS_CHROOT/" ]; then
      sudo umount -l "$ARCH_TRAVIS_CHROOT$user_home/$(basename "$d")"
    fi
  done
  sudo umount -l $ARCH_TRAVIS_CHROOT
